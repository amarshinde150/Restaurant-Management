...
db.connect(() => {
    ...
    sql = `create table if not exists employee_details(id int primary key AUTO_INCREMENT,email varchar(40),name varchar(30),mobile varchar(20) default "9876543210", password varchar(200))`
    db.query(sql, (err, result) => {
        if (err) throw err;
        // console.log(result);
    })
    ...
    sql = `create table if not exists food_category(category_id int primary key AUTO_INCREMENT,category_name varchar(40),item_count int,description varchar(20))`
    db.query(sql, (err, result) => {
        if (err) throw err;
        // console.log(result);
    })
    sql = `create table if not exists cart(cart_id int primary key AUTO_INCREMENT,profile_id int,item_name varchar(50),item_price int,item_quantity int,total int,foreign key(profile_id) references users_details(id))`;
    db.query(sql, (err, result) => {
        if (err) throw err;
        // console.log(result);
    })
            ...
            result.forEach((ele, ind) => {
                content += `<div id='${ind + 1000}' class="itemCard">
                        <a
                            ...
                            <div class="lqzIz w">
                                <img src="${ele.image_name}" style="height: 135px; width: 100%; object-fit: cover;">
                            </div>
                        </a>
                        <div class="gbprQ">
...
module.exports.addCategory = (req, res) => {
    ...
    if (req.myfile == null || req.myfile == '') {
        req.myfile = `https://media-cdn.tripadvisor.com/media/photo-s/17/9d/80/e6/dinner-buffet.jpg`;
    }
    else {
        req.myfile = '/' + req.myfile;
    }
    if (req.ownerIsHere) {
        sql = "insert into food_category(category_name,description,image_name) values(?,?,?)";
        db.query(sql, [name, desc, req.myfile], (err, result) => {
            if (err) {
...
module.exports.addFoodItem = (req, res) => {
    ...
    if (req.ownerIsHere) {
        sql = `insert into food_items(item_name,item_price,item_type,c_id,image_name) values(?,?,?,?,?)`;
        db.query(sql, [name, price, type, c_id, req.myfile], (err, result) => {
            if (err) throw err;
            // console.log(result);
            ...
            content += `<form action="/auth/updateQuantityValue" method="post">
                <tr>
                    ...
                    <td>
                      ...
                      <button> <a href="/auth/email?id=${id}" style="color:white">Order</a></button>
                </div>
            </div>`;
            // var details = {}
            // details.name = req.data.name;
...
module.exports.email = (req, resu) => {
    id = req.query.id;
    db.query('select * from users_details where id = ?', [req.query.id], (err, res) => {
        to = res[0].email;
        subject = "Your Order Confirmation\n";
        body = "Your order details\n";
        db.query('select * from cart where profile_id = ?', [id], (err, res1) => {
            var total = 0;
            res1.forEach(element => {
                body += element.item_name + "  " + element.item_price + "Rs/item  " + element.item_quantity + '-items\n';
                total += element.total;
            });
            db.query('delete from cart where profile_id = ?', [id], (err, res2) => {
                console.log("Deleted");
            })
            setTimeout(() => {
                body += "\n Total Bill Is : " + total;
                var transporter = nodemailer.createTransport({
                    service: "gmail",
                    auth: {
                        user: "eat.order.ez@gmail.com",
                        pass: "EatEzOrder",
                    },
                });
                var mailOptions = {
                    from: "eat.order.ez@gmail.com",
                    to: to,
                    subject: subject,
                    text: body,
                };
...
module.exports.viewEmployee = (req, res) => {
    ...
    <tbody>`
    db.query(`select * from employee_details`, (err, resul) => {
    ...
    setTimeout(() => {
        res.render('viewEmployee', { content: content });
    }, 3000);
}
module.exports.viewCustomer = (req, res) => {
    var id = req.query.id;
    var details = {}
    content = `<table class="table table-striped">
    <thead>
    ...
    <tbody>`
    db.query(`select * from users_details`, (err, resul) => {
        resul.forEach(element => {
            content += `<tr>
                <td>${element.id}</td>
    ...
    setTimeout(() => {
        res.render('viewCustomer', { content: content });
    }, 3000);
}